export default {
  async fetch(request, env) {
    ...
    if (secret === env.SECRET_PROZHARIM) site = "ПРОЖАРИМ";
    else if (env.SECRET_SUSHIDZA && secret === env.SECRET_SUSHIDZA) site = "СУШИДЗА";
    else if (env.SECRET_BANZAI && secret === env.SECRET_BANZAI) site = "БАНЗАЙ";
    else return json({ error: "Unauthorized" }, 401, corsHeaders);
...
function formatOrder(p, site) {
  ...
  const orderNo = dt.getFullYear().toString() + pad(dt.getMonth()+1) + pad(dt.getDate()) + "-" + pad(dt.getHours()) + pad(dt.getMinutes());
  const time = `${pad(dt.getDate())}.${pad(dt.getMonth()+1)}.${dt.getFullYear()} ${pad(dt.getHours())}:${pad(dt.getMinutes())}`;
  ...
  if (p.when?.type === "later" && p.when?.date) {
    ...
    if (!Number.isNaN(whenDt.getTime())) {
      whenText = `${pad(whenDt.getDate())}.${pad(whenDt.getMonth()+1)}.${whenDt.getFullYear()} ${pad(whenDt.getHours())}:${pad(whenDt.getMinutes())}`;
    }
  }
  ...
  for (let i = 0; i < (p.items || []).length; i++) {
    ...
    if (totalLength + line.length > 2200) { hidden++; continue; }
    items.push(line);
    totalLength += line.length;
  }
  if (hidden > 0) items.push(`... и ещё ${hidden} поз.`);

  const itemsBlock = items.length ? items.map((line, i) => i === items.length - 1 ? `└ ${line}` : `├ ${line}`).join("
") : "└ —";

  const receiveBlock = [];
  receiveBlock.push(`├ Способ: ${deliveryType}`);
  receiveBlock.push(`├ На когда: ${whenText}`);
  if (p.pricing?.deliveryModeLabel) receiveBlock.push(`├ Тариф доставки: ${p.pricing.deliveryModeLabel}`);
  ...
  if (p.cutlery?.count) {
    ...
    if ((p.cutlery?.fee || 0) > 0) extraBlock.push(`├ Платные приборы: ${p.cutlery.paid || p.cutlery.paidCount || 0} шт. • ${p.cutlery.fee || p.cutlery.price || 0} ₽`);
  }
  if ((p.pricing?.nightMarkup || 0) > 0) {
    extraBlock.push(`└ Ночь +10%: ${p.pricing.nightMarkup} ₽`);
  } else if (extraBlock.length) {
    extraBlock[extraBlock.length - 1] = extraBlock[extraBlock.length - 1].replace(/^├/, "└");
  }

  const paymentBlock = [];
  paymentBlock.push(`├ Способ: ${paymentLabel}`);
  if (p.payment === "cash" && (p.cashChange || p.changeFrom)) paymentBlock.push(`└ Сдача с: ${p.cashChange || p.changeFrom}`);
  else paymentBlock[0] = paymentBlock[0].replace(/^├/, "└");
