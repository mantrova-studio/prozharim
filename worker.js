...
function formatOrder(p, site) {
  ...
  const orderNo = `${dt.getFullYear()}${pad(dt.getMonth() + 1)}${pad(dt.getDate())}-${pad(dt.getHours())}${pad(dt.getMinutes())}`;
  ...
  for (let i = 0; i < (p.items || []).length; i++) {
    ...
    if (totalLength + line.length > 2200) hidden++;
    else { items.push(line); totalLength += line.length; }
  }
...
`;
  ...
  const cutleryCount = Number(p.cutlery?.count || 0);
  const cutleryPaid = Number(p.cutlery?.paid || 0);
  const cutleryFee = Number(p.cutlery?.fee || 0);
...
") || "—",
    ``,
    `🍴 Приборы: ${cutleryCount}` + (cutleryPaid > 0 ? ` (платных ${cutleryPaid} на ${cutleryFee} ₽)` : ""),
    `💰 Сумма блюд: ${subtotal} ₽`,
    `🧾 Итого: ${total} ₽`,
    `💳 Оплата: ${paymentText}`,
    ...(p.cashChange ? [`💵 Сдача с: ${p.cashChange} ₽`] : []),
    ``,
    `💬 Комментарий:`,
    p.comment || "—"
  ].join("
