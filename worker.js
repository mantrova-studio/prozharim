...
function cutleryLines(cutlery) {
  const count = Number(cutlery?.count || 0);
  const fee = Number(cutlery?.fee || 0);
  const free = Number(cutlery?.free || 0);
  const paid = Number(cutlery?.paid || 0);

  if (!count) return [`🍴 Приборы: 0`];

  const lines = [`🍴 Приборы: ${count}`];
  if (paid > 0) {
    lines.push(`🍴 Платные приборы: ${paid} × 6 ₽ (${free} бесплатно)`);
    lines.push(`🍴 Сумма за приборы: ${fee} ₽`);
  }
  return lines;
}
