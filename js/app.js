...
function cutleryCount(){
  const n = Number(els.cutleryInput?.value || state.cutlery || 0);
  if (!Number.isFinite(n) || n < 0) return 0;
  return Math.floor(n);
}

function cutleryFee(){
  const extra = Math.max(0, cutleryCount() - 5);
  return extra * 6;
}
...
function setupPaymentUI(){
  ...
  if (els.paymentSelect) {
    els.paymentSelect.addEventListener("change", update);
  }

  if (els.cutleryInput) {
    els.cutleryInput.addEventListener("input", () => {
      let n = String(els.cutleryInput.value || "").replace(/\D/g, "");
      if (n === "") n = "0";
      els.cutleryInput.value = String(Math.max(0, parseInt(n, 10) || 0));
      state.cutlery = cutleryCount();
      renderTotals();
    });
  }
