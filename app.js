...
function setupPaymentFields(){
  const select = els.paymentSelect || els.checkoutForm?.elements?.payment;
  const block = els.cashChangeBlock;
  const input = els.checkoutForm?.elements?.cashChange;
  if (!select || !block) return;

  const sync = () => {
    const isCash = select.value === "cash";
    block.hidden = !isCash;
    block.style.display = isCash ? "block" : "none";
    if (!isCash && input) input.value = "";
  };
...
function buildOrderPayload(form){
  ...
  return {
    ...
    cutlery: Number(form.cutlery?.value || 0),
