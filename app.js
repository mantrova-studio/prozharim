...
let MENU = [];
let ZONES = null;
const ZONES_FILE_DAY = "data/zones_day.geojson";
const ZONES_FILE_NIGHT = "data/zones_night.geojson";
let currentZonesMode = null;
let ymap = null;
let ymarker = null;
...
function openCheckout(){
  ...
  if (state.mode === "delivery") {
    refreshZonesForCurrentTime().catch(() => {});
    refreshZonesForCurrentTime().catch(() => {});
    ensureMap().catch(() => {});
  }
}
...
function getZonesModeByTime(){
  const hour = getOrenburgHour();
  return (hour >= 11 && hour < 23) ? "day" : "night";
}

async function loadZonesByCurrentTime(force = false){
  const nextMode = getZonesModeByTime();
  if (!force && currentZonesMode === nextMode && ZONES) return ZONES;

  const zonesUrl = nextMode === "day" ? ZONES_FILE_DAY : ZONES_FILE_NIGHT;
  const fallbackUrl = "data/zones.geojson";

  let loaded = null;

  try {
    const res = await fetch(zonesUrl, { cache: "no-store" });
  ...
  ZONES = loaded;
  currentZonesMode = nextMode;
  return ZONES;
}

async function refreshZonesForCurrentTime(){
  const previousMode = currentZonesMode;
  await loadZonesByCurrentTime(false);
  if (previousMode && previousMode !== currentZonesMode && state.delivery.lat != null && state.delivery.lng != null) {
    await setDeliveryPoint(state.delivery.lat, state.delivery.lng, state.delivery.address || null, false);
  }
}
...
function renderTotals(){
  ...
  if (state.mode === "delivery"){
    if (state.delivery.available && typeof state.delivery.price === "number"){
      ...
      els.sumTotal.textContent = rub(productSum + cutlerySum + state.delivery.price);
    } else {
      els.sumDelivery.textContent = "Недоступно";
      els.sumTotal.textContent = rub(productSum + cutlerySum);
    }
  } else {
...
function buildOrderPayload(form){
  ...
  const total = subtotal + (delivery.price || 0) + cutleryFee();
  ...
  return {
    ...
    meta: {
      ...
      zonesMode: currentZonesMode || getZonesModeByTime()
    }
  };
}
...
async function init(){
  ...
  await loadZonesByCurrentTime(true);
